...
export default function IndividualProfilePage({ name }: Props) {
  const vehicles = subscriptions.filter((s) => s.customerName === name)

  const [editing, setEditing] = useState(false)
  const [contact, setContact] = useState(vehicles.length ? vehicles[0].phone : "")
          ...
          {editing && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              ...
              <div className="sm:col-span-3 flex justify-end mt-2">
                <Button onClick={() => { console.log('Save individual', { name, contact, email, address }); setEditing(false); }}>Save</Button>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {vehicles.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="text-xs text-muted-foreground">No vehicles found.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
