...
import { companies, subscriptions, type Subscription, type SubscriptionStatus } from "@/data/dummy"
import { Building2, Phone, Mail, MapPin, Car, Pencil, Save, X, Hash } from "lucide-react"
...
const planOptions = ["Basic", "Standard", "Premium", "Fleet"]
const statusOptions: SubscriptionStatus[] = ["Active", "Due Soon", "Overdue", "Suspended"]

export default function CompanyProfile({ id, onClose }: CompanyProfileProps) {
  const co = companies.find((c) => c.id === id)
  const source = co ? subscriptions.filter((s) => s.customerName === co.companyName) : []

  const [vehicles, setVehicles] = useState<EditableVehicle[]>(() => source.map((v) => ({ ...v })))
  const [editingId, setEditingId] = useState<string | null>(null)

  const totalMonthly = vehicles.reduce((sum, v) => sum + v.monthlyAmount, 0)

  const updateField = (vid: string, field: keyof Subscription, value: string | number) => {
    setVehicles((prev) =>
      prev.map((v) => (v.id === vid ? { ...v, [field]: value, dirty: true } : v))
    )
  }
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
              ...
              <p className="font-semibold text-sm mt-1 truncate">{co.billingContactName ?? "-"}</p>
            </div>
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
              ...
              <p className="text-sm flex items-center gap-1.5 mt-1 truncate">
                <Phone size={11} className="text-muted-foreground shrink-0" /> {co.contactPhone ?? "-"}
              </p>
            </div>
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
              <p className="text-[0.6rem] uppercase tracking-widest text-muted-foreground font-semibold">Email</p>
              <p className="text-sm flex items-center gap-1.5 mt-1 truncate">
                <Mail size={11} className="text-muted-foreground shrink-0" /> {co.email ?? "-"}
              </p>
            </div>
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
              <p className="text-[0.6rem] uppercase tracking-widest text-muted-foreground font-semibold">Address</p>
              <p className="text-sm flex items-center gap-1.5 mt-1 truncate">
                <MapPin size={11} className="text-muted-foreground shrink-0" /> {co.address ?? "-"}
              </p>
            </div>
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
              ...
              <p className="text-sm flex items-center gap-1.5 mt-1">
                <Hash size={11} className="text-muted-foreground shrink-0" /> {co.taxId ?? "-"}
              </p>
            </div>
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
          ...
          <div className="flex items-center justify-between rounded-md border border-border bg-muted/20 px-4 py-2">
            <span className="text-xs text-muted-foreground">Total Amount Owed</span>
            <span className="font-bold text-base">GH₵ {totalMonthly.toLocaleString()}</span>
          </div>
                        ...
                        <tr key={v.id} className="hover:bg-muted/20 transition-colors">
                          <td className="px-4 py-2.5 whitespace-nowrap">
                            {isEditing ? (
                              <Input
                                value={v.plateNumber}
                                onChange={(e) => updateField(v.id, "plateNumber", e.target.value)}
                          ...
                          <td className="px-4 py-2.5 whitespace-nowrap">
                            {isEditing ? (
                              <Input
                                value={v.imei}
                                onChange={(e) => updateField(v.id, "imei", e.target.value)}
                                className="h-8 text-xs w-40 font-mono"
                              />
                            ) : (
                              <span className="font-mono text-xs text-muted-foreground">{v.imei}</span>
                            )}
                          </td>
                          <td className="px-4 py-2.5 whitespace-nowrap">
                            {isEditing ? (
                              <Select value={v.plan} onValueChange={(val) => updateField(v.id, "plan", val)}>
                          ...
                          <td className="px-4 py-2.5 whitespace-nowrap">
                            {isEditing ? (
                              <Input
                                type="date"
                                value={v.expiryDate}
                                onChange={(e) => updateField(v.id, "expiryDate", e.target.value)}
                                className="h-8 text-xs w-36"
                              />
                            ) : (
                              <span className="text-sm text-muted-foreground">{v.expiryDate}</span>
                            )}
                          </td>
                          <td className="px-4 py-2.5 whitespace-nowrap">
                            {isEditing ? (
                              <Select
                                value={v.status}
                                onValueChange={(val) => updateField(v.id, "status", val)}
                              >
                                <SelectTrigger className="h-8 text-xs w-28">
                                  <SelectValue />
                                </SelectTrigger>
                              ...
                              <Badge variant="outline" className={`text-[0.65rem] font-semibold ${statusColors[v.status] ?? ""}`}>
                                {v.status}
                              </Badge>
                            )}
                          </td>
                          <td className="px-4 py-2.5 text-right whitespace-nowrap">
                            {isEditing ? (
                              <Input
                                ...
                                onChange={(e) => updateField(v.id, "monthlyAmount", Number(e.target.value))}
                                className="h-8 text-xs w-24 ml-auto"
                              />
                            ) : (
                              <span className="font-semibold text-sm">GH₵ {v.monthlyAmount}</span>
                            )}
                          </td>
                          <td className="px-3 py-2.5 whitespace-nowrap">
                            {isEditing ? (
                              <div className="flex gap-0.5">
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-emerald-600 hover:text-emerald-700" onClick={() => handleSave(v.id)}>
                                  <Save size={13} />
                                </Button>
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-600" onClick={handleCancel}>
                                  <X size={13} />
                                </Button>
                              </div>
                            ) : (
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={() => setEditingId(v.id)}>
                                <Pencil size={13} />
                              </Button>
                            )}
                          </td>
