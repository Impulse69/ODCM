...
export default function CompanyProfile({ id, onClose }: CompanyProfileProps) {
  ...
  const source = co
    ? subscriptions.filter((s) => s.customerName === co.companyName)
    : [];
  ...
  const updateField = (
    ...
    setVehicles((prev) =>
      prev.map((v) =>
        v.id === vid ? { ...v, [field]: value, dirty: true } : v,
      ),
    );
  };
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
            ...
            <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
              ...
              <p className="font-bold text-sm flex items-center gap-1.5 mt-1">
                <Car size={11} className="text-muted-foreground shrink-0" />{" "}
                {vehicles.length}
              </p>
            </div>
          </div>
